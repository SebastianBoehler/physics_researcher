...
from autolab.simulators.manual_protocol import ManualProtocolSimulator
...
def build_default_registry(settings: Settings) -> SimulatorRegistry:
    ...
    registry.register(SimulatorKind.MANUAL_PROTOCOL, ManualProtocolSimulator(settings=settings))
    registry.register(SimulatorKind.CSV_MEASUREMENT, CSVMeasurementSimulator(settings=settings))
    return registry
